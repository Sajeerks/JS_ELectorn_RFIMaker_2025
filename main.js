...
ipcMain.on("openTheContinousFileDialogFOrRFIFRONTSHEETs", async (event, args) => {
  ...
  await dialog
    .showOpenDialog(win, {
      title: `select the pdf file RFI frontsheets `,
...
console.log("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrr");
         ...
         try {
          ...
          fs.writeFile( dirname+"//"+name+'--output.pdf', await pdfDoc.save(),(err) => {
            if (err)
              console.log(err);
            else {
         ...
         bufferArr.push(existingPdfBytes)
         await unlink(ArrayOFRFIFrontSheetALLFIlePaths[i]), (err => {
  ...
  // win.webContents.send("markPDFsFrommain", ArrayOFRFIFrontSheetALLFIlePaths)
...
return promise1
      ...
      console.log("senong thennnnnnnnnnnnnnnnnnnnnnn");
      console.log(value);
    })
    .catch((err) => {
      console.log(err);
...
ipcMain.on("combineAllTheFilesToone", async (event, args) => {
  // const webContents = event.sender

  console.log("combineallliles");
  console.log(args);
...
// ipcMain.handle('selcalleingafterGeetingFrontFilePDfs',async (event, args) => {
...
async function delayaer(args){
  // console.log(args);
 await editPDFfileswriteNumbers(args)
}


     
 function editPDFfileswriteNumbers(ArrayOFRFIFrontSheetALLFIlePaths){

  for (let i = 0; i < ArrayOFRFIFrontSheetALLFIlePaths.length; i++) {

    let folderPath=    path.dirname(ArrayOFRFIFrontSheetALLFIlePaths[i])

    let filename = path.parse(ArrayOFRFIFrontSheetALLFIlePaths[i]).name 
     console.log(folderPath);
console.log(filename);

console.log('After', i, 'th API call');
           delayforTaggingPDF(ArrayOFRFIFrontSheetALLFIlePaths[i]);
